...
describe('monthAsNum and monthAsStr', () => {
    it('converts between month strings and numbers as inverse operations', () => {
        // monthAsNum converts "01"-"12" to 0-11 (JavaScript month indices)
        // monthAsStr converts 0-11 to "01"-"12" (padded strings)

        expect(monthAsNum('01')).toBe(0); // January
        expect(monthAsStr(0)).toBe('01');

        expect(monthAsNum('06')).toBe(5); // June
        expect(monthAsStr(5)).toBe('06');

        expect(monthAsNum('12')).toBe(11); // December
        expect(monthAsStr(11)).toBe('12');

        // The two functions are inverses of each other for all months
        for (let i = 0; i < 12; i++) {
            const str = monthAsStr(i);
            expect(monthAsNum(str)).toBe(i);
        }
    });
});
...
describe('strAsDate and dateAsStr', () => {
    ...
    it('handles undefined and invalid inputs correctly', () => {
        ...
        expect(strAsDate('2025-01')).toBeUndefined(); // Missing day
        expect(strAsDate('2025/01/15')).toBeUndefined(); // Wrong separator

        // JavaScript Date allows invalid months/days and rolls them over
        expect(strAsDate('2025-13-01')).toBeDefined(); // Month 13 rolls to Jan 2026
        expect(strAsDate('2025-01-32')).toBeDefined(); // Day 32 rolls to Feb 1
    });
});
